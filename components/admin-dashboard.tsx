  ...
  return (
    <div className="admin-form-grid">
      ...
      <p className="admin-form-note full">
        Set both prices to 0 for a free issue. Paid issues use the server-side
        PayPal checkout when its runtime secrets and database are connected.
      </p>
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([me, store]) => {
        ...
        if (store.settings) {
          setPaymentsReady(
            store.settings.paymentsEnabled ||
              store.settings.subscriptionsEnabled,
          );
          setPassForm({
    ...
    <div className="admin-dashboard">
      <div className="admin-summary">
        ...
        <div className="payment-status">
          <span>Payments</span>
          <strong>{paymentsReady ? "SANDBOX" : "OFF"}</strong>
        </div>
      </div>
